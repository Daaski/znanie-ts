...
export const EditEvent = ({ mode, event }: EditEventProps) => {
    ...
    const {
        ...
        initialValues: {
            pk: event?.pk ?? 0,
            start: new Date(event?.start ? event.start : Date.now()),
            end: new Date(event?.end ? event.end : Date.now()),
            name: event?.name ?? '',
            address: {
                id: event?.pk ? event.pk : 0,
                name: event?.address
                    ? `${event.address.subject}, ${event.address.type} ${event.address.name}`
                    : '',
            },
            status: event?.status ? event.status : 'new',
            image: event?.image ? event.image : '',
            about: event?.about ? event.about : '',
            description: event?.description ? event.description : '',
            lectors: !!event?.lectors?.length
                ? event?.lectors.filter((lector) => +lector.pk !== +user.pk)
                : [],
        },
        onSubmit,
    ...
    return (
        <form className={css.event_form_container} onSubmit={handleSubmit}>
            ...
            <InputSelect
                ...
                autoComplete="off"
                which="address/"
                handleError={touched.address && errors.address}
            />
            <InputDate
                value={values.start}
            ...
            <div className={css.form_input_lector}>
                <InputLector
                    ...
                    onBlur={handleBlur}
                    name="lectors"
                    handleError={touched.lectors && errors.lectors}
